...
## Running on the Lab iMac Workstations

The lab iMac workstations (16 GB RAM, Apple Silicon) are sufficient for this stage of the project. The training script automatically detects and uses the **Apple MPS GPU**, which is significantly faster than CPU-only training.

### System Requirements Check

| Resource | Lab iMac | Required | Status |
|----------|----------|----------|--------|
| RAM | 16 GB | ~6–8 GB (batch 16) |  Sufficient |
| GPU | Apple MPS | Optional, but faster |  Auto-detected |
| Storage | — | ~2–5 GB for 10k–20k PNGs | Sufficient |
| Python | — | 3.8+ |  |
...
Python 3.8 or higher is required. If not installed, use Homebrew:
...
After activation, `(venv)` will appear at the start of your terminal prompt. All packages installed from this point will be scoped to this environment.
...
PyTorch on macOS includes MPS support by default — no separate CUDA installation is needed.

To verify that MPS is available after installation:
...
`organized_data/` is produced by the data organisation scripts (`organize_from_json.py` or `organize_single_bundle.py`).

### Step 5 — Run Training

The recommended settings for the lab iMac use a smaller batch size to stay comfortably within 16 GB RAM:
...
To do a quick sanity check that the pipeline runs end-to-end before committing to a full training run, set `--epochs 1`:

```bash
python3 train_chexpert.py --epochs 1 --batch_size 16 --num_workers 2
```
...
[train] 14000 images across 14 classes
[val]   2000 images across 14 classes
[test]  4000 images across 14 classes

──────────────────────────────────────────────────
  Epoch 1/15  |  LR: 1.00e-04
──────────────────────────────────────────────────
  Epoch 1 | Batch 50/875 | Loss: 0.3241 | Time: 42.3s
  ...
  → Train Loss: 0.2891 | Val Loss: 0.2743 | Val AUC: 0.7812
  ✓ New best model saved (AUC=0.7812)
```

### Estimated Training Time

Based on a subset of ~10k–20k images at 224×224:

| Device | Time per Epoch | 15 Epochs Total |
|--------|---------------|-----------------|
| iMac (Apple MPS) | ~15–30 min | ~4–7 hours |
| iMac (CPU only) | ~60–90 min | ~15–22 hours |
| Google Colab (GPU) | ~3–8 min | ~1–2 hours |

The lab iMac with MPS is suitable for initial experiments. If faster iteration is needed later, Google Colab (free GPU, ~12 GB VRAM) is a practical option without requiring any code changes — the device detection logic in the script handles this automatically.
